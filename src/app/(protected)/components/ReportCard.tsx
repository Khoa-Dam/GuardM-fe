...
import { VerificationLevel, VerificationCrimeReport } from '../../../types/map';
...
const levelBadgeMap: Record<VerificationLevel, { label: string; classes: string }> = {
...
export const ReportCard: React.FC<ReportCardProps> = ({ report, onClose, onConfirm, onDispute }) => {
    const badge = levelBadgeMap[report.verificationLevel];
                ...
                <div className="p-4 overflow-y-auto max-h-[60vh]">
                    ...
                    <div className="mb-4 p-3 rounded-lg border border-slate-100 bg-white shadow-sm">
                        <div className="flex justify-between text-xs mb-1.5 font-bold text-gray-700">
                            ...
                            <span className={report.trustScore >= 70 ? 'text-green-600' : 'text-gray-600'}>{report.trustScore}/100</span>
                        </div>
                        <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                            <div className={`h-full transition-all duration-700 ease-out ${scoreColor(report.trustScore)}`} style={{ width: `${report.trustScore}%` }} />
                        </div>
                        <div className="mt-2.5 flex justify-between text-[11px] text-gray-500 border-t border-gray-50 pt-2">
                            <span className="flex items-center gap-1.5">
                                <span className="w-1.5 h-1.5 rounded-full bg-green-500" /> {report.confirmationCount} Xác nhận
                            </span>
                            <span className="flex items-center gap-1.5">
                                <span className="w-1.5 h-1.5 rounded-full bg-red-500" /> {report.disputeCount} Báo sai
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-lg text-sm font-semibold active:scale-95 shadow-blue-100 shadow-lg"
                        >
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-2 bg-white hover:bg-gray-50 text-gray-700 border border-gray-200 py-2.5 rounded-lg text-sm font-semibold active:scale-95"
                        >
                            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
